//filter even no students 
// /filter based on class(8,9,10)
// /filter based on grades(A,B,C)
//reduce
// calculate the total COntri
//map
// update the grades by one level
...
function filterByClass(arr , ele){
    return arr.filter((el)=>el.class === ele)
}
...
function filterbyGrade(arr , grade){
    return arr.filter((el)=>el.grade.toLowerCase() 
    === grade.toLowerCase())
}
